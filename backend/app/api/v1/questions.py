...
async def get_questions(
    ...
    result = await db.execute(
        ...
        .options(
            ...
            selectinload(Question.knowledge_node)
        )
        .where(*filters)
...
async def get_question_similarity(question_id: int, threshold: float = 0.3, limit: int = 10, db: AsyncSession = Depends(get_db)):
    ...
    target_skill_id = target_q.knowledge_node_id
    if not target_skill_id:
        return []
        
    all_q_result = await db.execute(
        select(Question).where(
            Question.knowledge_node_id == target_skill_id,
...
async def create_question(
    ...
    result = await db.execute(
        select(Question).options(selectinload(Question.answers), selectinload(Question.sub_items), selectinload(Question.knowledge_node)).where(Question.id == db_question.id)
    )
    capture(
        request,
        "question_created",
...
async def update_question(
    ...
    if existing_q.status == QuestionStatus.APPROVED:
        ...
        new_q = Question(
            ...
            creator_id=current_user.id,
            status=QuestionStatus.PENDING,
            parent_question_id=existing_q.id,
        ...
        if q_in.knowledge_node_id is not None:
            if not await KnowledgeService.is_leaf(db, q_in.knowledge_node_id):
                raise HTTPException(status_code=400, detail="Câu hỏi chỉ được gắn vào node lá (node không có con).")
...
async def review_question(
    ...
    result = await db.execute(select(Question).options(selectinload(Question.answers), selectinload(Question.sub_items), selectinload(Question.knowledge_node)).where(Question.id == question_id))
    question = result.scalars().first()
    if not question:
        raise HTTPException(status_code=404, detail="Question not found")
...
async def get_question_history(question_id: int, db: AsyncSession = Depends(get_db)):
    ...
    while curr:
        res = await db.execute(select(Question).options(selectinload(Question.answers), selectinload(Question.sub_items), selectinload(Question.knowledge_node)).where(Question.id == curr))
    ...
    while queue:
        curr_id = queue.pop(0)
        res = await db.execute(select(Question).options(selectinload(Question.answers), selectinload(Question.sub_items), selectinload(Question.knowledge_node)).where(Question.parent_question_id == curr_id))
