...
import { ArrowLeft, BarChart2, CheckSquare, Search } from "lucide-react";
...
export default function ExamComparePage() {
  ...
  const handleSelect = (id: number) => {
    setSelectedExamIds(prev => {
      ...
      if (prev.length >= 4) {
        alert("Chỉ so sánh tối đa 4 đợt thi cùng lúc để biểu đồ hiển thị tốt nhất.");
        return prev;
      }
      return [...prev, id];
    });
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      await Promise.all(selectedExamIds.map(async (id) => {
        if (!examStats[id]) {
          const res = await fetch(`http://localhost:8000/api/v1/statistics/exams/${id}/overview`);
          if (res.ok) {
  ...
  const filteredExams = useMemo(() => {
    return exams.filter(e => e.name.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [exams, searchTerm]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate("/admin/exams")} className="rounded-full bg-white/80 dark:bg-slate-800/80 shadow-sm border border-slate-200/60 dark:border-slate-700/60 hover:bg-slate-100 dark:hover:bg-slate-700 backdrop-blur-md">
          <ArrowLeft className="w-5 h-5" />
        </Button>
        <div>
          <h1 className="text-3xl font-extrabold text-gradient pb-1">So sánh Kỳ thi</h1>
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">Phân tích chênh lệch chất lượng điểm số giữa các đợt thi</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <Card className="p-0 overflow-hidden glass-card shadow-lg border border-slate-200/60 dark:border-slate-700/60 xl:col-span-1 h-[600px] flex flex-col">
          <div className="p-4 bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 font-semibold text-slate-800 dark:text-slate-200 flex justify-between items-center shrink-0">
            <span>Chọn kỳ thi <span className="ml-1 inline-flex items-center justify-center rounded-full bg-primary-100 dark:bg-primary-900/50 px-2 py-0.5 text-[10px] font-bold text-primary-700 dark:text-primary-400">{selectedExamIds.length}/4</span></span>
            <Button size="sm" onClick={handleCompare} disabled={selectedExamIds.length === 0 || isComparing} className="shadow-lg shadow-primary-500/20">
          ...
          <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
            {filteredExams.length === 0 && (
              <div className="p-8 text-center flex flex-col items-center gap-2 opacity-60">
                ...
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Không tìm thấy kỳ thi nào.</p>
              </div>
            )}
            {filteredExams.map(exam => (
              <label key={exam.id} className={`group flex items-start gap-3 p-3 mb-1 cursor-pointer rounded-xl transition-all border ${selectedExamIds.includes(exam.id) ? 'bg-primary-50 dark:bg-primary-500/10 border-primary-200 dark:border-primary-500/30 shadow-sm' : 'border-transparent hover:bg-slate-100 dark:hover:bg-slate-800/60'}`}>
                <input 
                  type="checkbox" 
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-semibold truncate transition-colors ${selectedExamIds.includes(exam.id) ? 'text-primary-900 dark:text-primary-300' : 'text-slate-700 dark:text-slate-200 group-hover:text-slate-900 dark:group-hover:text-white'}`}>{exam.name}</p>
          ...
          <Card className="p-6 glass-card shadow-lg border border-slate-200/60 dark:border-slate-700/60 h-[400px] flex flex-col relative overflow-hidden group">
            ...
            {chartData.length > 0 ? (
              <div className="relative z-10 flex-1 w-full min-h-[250px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }} barGap={6}>
                    ...
                    <Bar dataKey="Trung bình" fill={COLORS.average} radius={[4, 4, 0, 0]} maxBarSize={40} />
                    <Bar dataKey="Cao nhất" fill={COLORS.max} radius={[4, 4, 0, 0]} maxBarSize={40} />
                    <Bar dataKey="Thấp nhất" fill={COLORS.min} radius={[4, 4, 0, 0]} maxBarSize={40} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ) : (
              <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500">
                ...
                <p className="font-medium">Vui lòng chọn kỳ thi và bấm "Xem biểu đồ"</p>
              </div>
            )}
          </Card>
          ...
          {chartData.length > 0 && (
            <Card className="p-0 overflow-hidden glass-card shadow-lg border border-slate-200/60 dark:border-slate-700/60 animate-in slide-in-from-bottom-4 duration-500">
              <div className="p-4 bg-slate-50/80 dark:bg-slate-900/50 border-b border-slate-200/60 dark:border-slate-800 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 dark:text-slate-100">Bảng Tổng hợp Cụ thể</h3>
              </div>
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-slate-50/50 dark:bg-slate-900/30 border-b border-slate-200/60 dark:border-slate-800 text-slate-500 dark:text-slate-400 uppercase tracking-wider text-[11px]">
                    <tr>
                      ...
                      <th className="px-6 py-4 font-bold text-right">Lượt nộp</th>
                      <th className="px-6 py-4 font-bold text-right">Trung bình</th>
                      <th className="px-6 py-4 font-bold text-right">Cao nhất</th>
                      <th className="px-6 py-4 font-bold text-right">Thấp nhất</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100/80 dark:divide-slate-800">
                    {selectedExamIds.map(id => {
                      ...
                      if (!stats || !exam) return null;
                      return (
                        <tr key={id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors group">
                          <td className="px-6 py-4">
                            <p className="font-bold text-slate-800 dark:text-slate-200">{exam.name}</p>
                          </td>
                          <td className="px-6 py-4 text-right font-medium text-slate-600 dark:text-slate-400">{stats.total_participants || 0}</td>
