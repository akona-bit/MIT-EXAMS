...
import { getQuestions, deleteQuestion, reviewQuestion, getQuestionSimilarity } from "../../api/questions";
import type { Question, QuestionSimilarityResponse } from "../../types";
import DataTable, { type Column } from "../../components/ui/DataTable";
import { Button } from "../../components/ui/Button";
import { Badge } from "../../components/ui/Badge";
import ConfirmDialog from "../../components/ui/ConfirmDialog";
import { Plus, Search, Edit3, Trash2, HelpCircle, CheckCircle, Copy, AlertCircle, History } from "lucide-react";
...
export default function QuestionsPage() {
  ...
  const [similarQuestions, setSimilarQuestions] = useState<QuestionSimilarityResponse[]>([]);
  ...
  const handleReview = async (approve: boolean) => {
    ...
    try {
      await reviewQuestion(reviewItem.id, approve, approve ? undefined : rejectReason);
      setReviewItem(null);
      setRejectReason("");
      await fetchQuestions();
    } catch (error: any) {
      console.error(error);
      setReviewError(error.response?.data?.detail || "Có lỗi xảy ra khi duyệt câu hỏi");
    } finally {
  ...
  const columns: Column<Question>[] = [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return <span className="text-xs">{typeLabels[row.type] || row.type}</span>;
      }
    },
    {
      key: "status",
      ...
      render: (row) => {
        ...
        return (
          <div className="flex flex-col gap-1 items-start">
            <Badge variant={variants[row.status] || "secondary"}>{row.status}</Badge>
            {row.status === 'REJECTED' && row.reject_reason && (
              <span className="text-[10px] text-danger-500 max-w-[120px] truncate" title={row.reject_reason}>
    ...
    },
    {
      ...
      header: "Sử dụng",
      render: (row) => (
        <Badge variant={row.usage_count && row.usage_count > 0 ? "info" : "secondary"}>
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="shrink-0 flex gap-2 flex-col sm:flex-row">
            ...
            <Link to="/admin/questions/new-group">
              <Button size="lg" variant="outline" className="w-full sm:w-auto hover:-translate-y-0.5 border-primary-500 text-primary-600">
                <Plus className="mr-2 h-5 w-5" />
                Thêm nhóm (Ngữ liệu)
              </Button>
            </Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
          <select className="border border-slate-300 rounded p-2 text-sm" value={filterLevel} onChange={e => setFilterLevel(e.target.value)}>
          ...
          <select className="border border-slate-300 rounded p-2 text-sm" value={filterType} onChange={e => setFilterType(e.target.value)}>
          ...
          <select className="border border-slate-300 rounded p-2 text-sm" value={filterPassage} onChange={e => setFilterPassage(e.target.value)}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Duyệt câu hỏi #{reviewItem.id}</h3>
              <p className="text-sm text-slate-500 mt-1">Quyết định phê duyệt hoặc từ chối câu hỏi này.</p>
            </div>
            ...
            <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-2">
              <Button variant="ghost" onClick={() => { setReviewItem(null); setRejectReason(""); setReviewError(""); }}>
                Hủy
              </Button>
              <Button 
                variant="destructive" 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Kiểm tra tương đồng</h3>
                <p className="text-sm text-slate-500 mt-1">Các câu hỏi giống với câu #{similarityItem.id}</p>
              </div>
              <Button variant="ghost" onClick={() => setSimilarityItem(null)}>Đóng</Button>
            </div>
            
            <div className="flex-1 overflow-auto p-6 space-y-6">
              <div className="space-y-2">
                <h4 className="text-sm font-semibold text-slate-900 dark:text-white">Câu hỏi gốc:</h4>
              ...
              <div>
                ...
                {isSimLoading ? (
                  <div className="flex items-center justify-center py-8 text-sm text-slate-500">Đang phân tích...</div>
                  ...
                  <div className="space-y-3">
                    {similarQuestions.map(sim => (
                      <div key={sim.question_id} className="p-4 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm flex flex-col sm:flex-row gap-4">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-slate-700 dark:text-slate-300 truncate">{sim.content}</p>
                          <div className="flex items-center gap-2 mt-2 text-xs text-slate-500">
                            <span className="font-mono">#{sim.question_id}</span>
                            <span>•</span>
                            <Badge variant="secondary" className="text-[10px]">{sim.status}</Badge>
                          </div>
                        </div>
                        <div className="shrink-0 flex flex-col items-end justify-center">
                          ...
                          <span className="text-[10px] uppercase tracking-wider text-slate-500">Độ giống</span>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {historyItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Lịch sử phiên bản</h3>
                <p className="text-sm text-slate-500 mt-1">Lịch sử thay đổi của câu hỏi #{historyItem.id}</p>
              </div>
              <Button variant="ghost" onClick={() => setHistoryItem(null)}>Đóng</Button>
            </div>
            
            <div className="flex-1 overflow-auto p-6 space-y-6">
              {isHistoryLoading ? (
                <div className="flex items-center justify-center py-8 text-sm text-slate-500">Đang tải lịch sử...</div>
                ...
                <div className="relative border-l-2 border-slate-200 dark:border-slate-700 ml-3 space-y-8 pb-4">
                  {historyList.map((hist, idx) => (
                    <div key={hist.id} className="relative pl-6">
                      <div className={`absolute -left-[9px] top-1 h-4 w-4 rounded-full border-2 border-white dark:border-slate-900 ${hist.id === historyItem.id ? 'bg-primary-500 ring-4 ring-primary-500/20' : 'bg-slate-300 dark:bg-slate-600'}`}></div>
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="font-semibold text-slate-900 dark:text-white text-sm">Phiên bản #{hist.id}</span>
                            {hist.id === historyItem.id && <Badge variant="info" className="text-[10px]">Hiện tại</Badge>}
                          </div>
                          <span className="text-xs text-slate-500">{new Date(hist.created_at || "").toLocaleString("vi-VN")}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-slate-500">Mức độ: {hist.level}</span>
                        </div>
                      </div>
                    </div>
                  ))}
