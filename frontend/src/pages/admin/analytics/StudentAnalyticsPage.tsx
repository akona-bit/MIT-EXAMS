  ...
  const getKnowledgeMatrix = () => {
    ...
    return (
      <div className="grid grid-cols-10 sm:grid-cols-15 gap-2 sm:gap-3 mt-6">
        {questions.map((q) => {
          let bg = "bg-slate-100 dark:bg-slate-800 text-slate-500 shadow-inner border border-slate-200/50 dark:border-slate-700";
          if (q.status === "correct") bg = "bg-success-500 text-white shadow-[0_0_15px_rgba(34,197,94,0.4)] border border-success-400";
          else if (q.status === "wrong") bg = "bg-danger-500 text-white shadow-[0_0_15px_rgba(239,68,68,0.4)] border border-danger-400";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border-b border-slate-200/60 dark:border-slate-800/60 pb-6">
        <h1 className="text-3xl font-extrabold text-gradient pb-1 tracking-tight">Tra Cứu Điểm Cá Nhân</h1>
        <p className="text-base font-medium text-slate-500 dark:text-slate-400 mt-2 max-w-2xl">
          Phân tích năng lực chuyên sâu, xác định lỗ hổng kiến thức qua bản đồ điểm số chuẩn hóa.
        </p>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <Card className="p-2 sm:p-4 glass-card border border-white/50 dark:border-white/10 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none overflow-hidden relative">
          ...
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4 relative z-10 p-2 sm:p-4">
            ...
            <Button type="submit" size="lg" className="h-14 rounded-2xl px-8 shadow-lg shadow-indigo-500/30 text-base font-bold tracking-wide">
              Tra cứu
            </Button>
          </form>
        </Card>
              ...
              <Card className="relative overflow-hidden glass-card p-8 rounded-3xl border border-white/50 dark:border-white/10 shadow-xl shadow-slate-200/50 dark:shadow-none">
                ...
                <div className="relative z-10">
                  <h2 className="text-sm font-bold text-indigo-500 dark:text-indigo-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                    <span className="h-2 w-2 rounded-full bg-indigo-500 animate-pulse" /> Hồ Sơ Thí Sinh
                  </h2>
                  
                  <div className="space-y-6">
                    <div>
                      <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Họ và tên</p>
                      <p className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight mt-1">{student.name}</p>
                    </div>
                    {responses && (
                      <div>
                        <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Tài khoản liên kết</p>
                        <p className="text-lg font-medium text-slate-700 dark:text-slate-300 mt-1">{responses.email || "Chưa có email"}</p>
                      </div>
                    )}
                    <div className="pt-6 mt-2 border-t border-slate-200/60 dark:border-slate-800/60 grid grid-cols-2 gap-6">
                      <div className="bg-white/50 dark:bg-slate-900/50 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 backdrop-blur-sm">
                        <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mb-2">Điểm Toán (IRT)</p>
                        <div className="flex items-end gap-2">
                          ...
                          <span className="text-sm text-slate-400 mb-1 font-bold">/300</span>
                        </div>
                      </div>
                      <div className="bg-white/50 dark:bg-slate-900/50 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 backdrop-blur-sm">
                        <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mb-2">Điểm TDKH (IRT)</p>
                        <div className="flex items-end gap-2">
              ...
              <Card className="p-8 glass-card rounded-3xl border border-white/50 dark:border-white/10 shadow-xl shadow-slate-200/50 dark:shadow-none relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="text-xl font-extrabold text-slate-900 dark:text-white">Ma Trận Kiến Thức</h3>
                  </div>
                  <div className="flex flex-wrap gap-4 mb-6 text-sm font-semibold text-slate-600 dark:text-slate-400 bg-white/60 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800/50">
                    <div className="flex items-center gap-2"><span className="w-4 h-4 rounded-full bg-success-500 shadow-[0_0_10px_rgba(34,197,94,0.3)]"></span> Trả lời đúng</div>
                    <div className="flex items-center gap-2"><span className="w-4 h-4 rounded-full bg-danger-500 shadow-[0_0_10px_rgba(239,68,68,0.3)]"></span> Trả lời sai</div>
                    <div className="flex items-center gap-2"><span className="w-4 h-4 rounded-full bg-slate-200 dark:bg-slate-700"></span> Bỏ trống</div>
                  </div>
                  {getKnowledgeMatrix()}
                </div>
              </Card>
            ...
            <motion.div variants={itemAnim} className="h-full">
              <Card className="p-8 h-[550px] flex flex-col glass-card rounded-3xl border border-white/50 dark:border-white/10 shadow-xl shadow-slate-200/50 dark:shadow-none relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center gap-3 mb-8">
                  ...
                  <h3 className="text-xl font-extrabold text-slate-900 dark:text-white">Phân tích năng lực (Radar)</h3>
                </div>
                <div className="flex-1 w-full min-h-0 relative z-10 -ml-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart cx="50%" cy="50%" outerRadius="75%" data={getRadarData()}>
                      <PolarGrid stroke={isDark ? "#334155" : "#e2e8f0"} strokeWidth={1.5} />
                      <PolarAngleAxis dataKey="subject" tick={{ fill: isDark ? '#94a3b8' : '#475569', fontSize: 13, fontWeight: 700, fontFamily: 'Inter' }} />
                      <PolarRadiusAxis angle={30} domain={[0, 300]} tick={{ fill: isDark ? '#64748b' : '#94a3b8', fontSize: 11 }} axisLine={false} />
                      <Radar name="Điểm của bạn" dataKey="Điểm của bạn" stroke="#6366f1" strokeWidth={3} fill="#6366f1" fillOpacity={0.4} />
                      <Radar name="Trung bình lớp" dataKey="Trung bình lớp" stroke={isDark ? "#475569" : "#94a3b8"} strokeWidth={2} strokeDasharray="5 5" fill={isDark ? "#334155" : "#cbd5e1"} fillOpacity={0.2} />
                      <Tooltip 
                        contentStyle={{ 
                      ...
                      />
                      <Legend 
                        wrapperStyle={{ paddingTop: '20px', fontFamily: 'Inter', fontWeight: 600 }} 
                        iconType="circle"
                      />
                    </RadarChart>
                  </ResponsiveContainer>
            ...
            <motion.div variants={itemAnim}>
              <Card className="p-8 bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-950/30 dark:to-amber-950/20 border-orange-200/50 dark:border-orange-900/50 shadow-lg shadow-orange-500/5 dark:shadow-none rounded-3xl relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-start gap-5">
                   ...
                   <div>
                     <h3 className="text-xl font-extrabold text-orange-900 dark:text-orange-100 mb-3 tracking-tight">Phân tích gợi ý ôn tập</h3>
                     <p className="text-base font-medium text-orange-800/80 dark:text-orange-200/80 leading-relaxed">
                       Dựa trên kết quả ma trận kiến thức, bạn nên tập trung cải thiện các phần kiến thức tương ứng với các câu trả lời sai (Màu đỏ). Đặc biệt lưu ý việc bỏ trống quá nhiều sẽ bị phạt và làm giảm mạnh điểm số trong mô hình IRT 2PL.
                     </p>
                   </div>
                </div>
              </Card>
