...
import { createKnowledgeNode, getKnowledgeGraph, deleteKnowledgeNode, updateKnowledgeNode, createManualLink, deleteManualLink } from "../../api/knowledge";
import type { KnowledgeGraphEdge, KnowledgeGraphNode } from "../../types";
import { useTheme } from "../../stores/themeStore";
import { motion, AnimatePresence } from "framer-motion";
import { Share2, FileText, Link as LinkIcon, Database, CheckCircle2, ChevronRight, Activity, Plus, ZoomIn, ZoomOut, Maximize, Minimize, Play, Pause, Target, X, Trash2, GitBranch, Timer } from "lucide-react";
...
function NodePill({
  ...
  return (
    ...
    >
      <div className={`relative flex h-8 w-8 shrink-0 items-center justify-center rounded-lg shadow-sm transition-all duration-300 ${active ? "scale-110 shadow-md" : "group-hover:scale-105"}`}
           style={{ backgroundColor: `${levelColors[node.type] ?? levelColors.NOTE}15`, color: levelColors[node.type] ?? levelColors.NOTE }}>
        <div className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: levelColors[node.type] ?? levelColors.NOTE, boxShadow: active ? `0 0 10px ${levelColors[node.type]}` : 'none' }} />
      </div>
      <span className="min-w-0 flex-1">
        <span className={`block truncate text-sm font-semibold transition-colors ${active ? "text-primary-700 dark:text-primary-300" : "text-slate-700 dark:text-slate-200 group-hover:text-slate-900 dark:group-hover:text-white"}`}>
          {node.label}
        </span>
      ...
      {node.question_count > 0 && (
        <span className={`flex h-5 items-center justify-center rounded-full px-2 text-[10px] font-bold transition-colors ${active ? "bg-primary-100 text-primary-700 dark:bg-primary-900/50 dark:text-primary-300" : "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400 group-hover:bg-slate-200 dark:group-hover:text-slate-700"}`}>
          {node.question_count}
        </span>
      )}
    </motion.button>
...
function GraphCanvas({
  ...
  useEffect(() => {
    if (replayProgress >= 0 && replayProgress < nodes.length) {
      ...
      }, Math.max(80, 4000 / nodes.length)); // scale speed, ~4 seconds total max
  ...
  const graphData = useMemo(() => {
    ...
    return {
      nodes: currentNodes.map(n => ({ ...n, id: n.id, name: n.label, val: (n.question_count || 1) * 1.5 })),
  ...
  const paintNode = useCallback((node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    let descWidth = 0;
    if (hasNote) {
      const raw = node.description.length > 40 ? node.description.slice(0, 38) + '…' : node.description;
      descText = raw;
    ...
    if (isSelected) {
      ...
      ctx.shadowColor = isDarkMode ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.1)';
    ...
    if (isSelected) {
      ctx.fillStyle = isDarkMode ? 'rgba(15, 23, 42, 0.96)' : 'rgba(255, 255, 255, 0.99)';
    } else if (isHovered) {
      ctx.fillStyle = isDarkMode ? 'rgba(30, 41, 59, 0.95)' : 'rgba(248, 250, 252, 0.98)';
    } else {
      ctx.fillStyle = isDarkMode ? 'rgba(15, 23, 42, 0.88)' : 'rgba(255, 255, 255, 0.94)';
    }
    ctx.fill();

    // Border — use nodeColor tint when selected
    ctx.strokeStyle = isSelected ? nodeColor : (isDarkMode ? 'rgba(51,65,85,0.5)' : 'rgba(226,232,240,0.7)');
    ...
    ctx.save();
    ctx.beginPath();
    ctx.roundRect(x, y, accentWidth + radius, totalHeight, [radius, 0, 0, radius]);
    ctx.clip();
    ...
    if (hasNote && descText) {
      ...
      ctx.fillText(descText, x + accentWidth + paddingX * 0.6, labelY + fontSize * 1.3);
    }
  ...
  return (
    <div className="relative w-full h-[600px] group">
      ...
      <div ref={containerRef} className="relative z-10 h-full w-full overflow-hidden rounded-2xl border border-slate-200/60 dark:border-slate-800/60 bg-[#fafbfd] dark:bg-[#080c14] shadow-inner transition-colors duration-300">
        {/* Dot grid background */}
        <div className="absolute inset-0 z-0 opacity-[0.35] dark:opacity-[0.12] pointer-events-none" style={{
        ...
        <div className="absolute inset-0 z-10">
          <ForceGraph2D
            ...
            linkColor={(link: any) => {
              const isActive = (selectedId && (link.source.id === selectedId || link.target.id === selectedId)) || 
                               (hoverNode && (link.source.id === hoverNode || link.target.id === hoverNode));
              const hasFocus = selectedId || hoverNode;
              
              const targetColor = levelColors[link.target?.type] ?? (isDarkMode ? '#94a3b8' : '#64748b');
              
              if (isActive) {
                return targetColor;
              }
              if (hasFocus) {
                return isDarkMode ? 'rgba(71, 85, 105, 0.08)' : 'rgba(203, 213, 225, 0.15)';
              }
            ...
            linkDirectionalParticleColor={(link: any) => {
              return levelColors[link.target?.type] ?? (isDarkMode ? '#94a3b8' : '#64748b');
            }}
        ...
        <div className="absolute right-4 top-4 z-20 flex flex-col gap-2">
          <div className="flex flex-col rounded-lg border border-white/60 dark:border-slate-700/50 bg-white/70 dark:bg-slate-800/70 shadow-sm backdrop-blur p-1">
            <button type="button" title="Tái hiện lịch sử hình thành (Timelapse)" className={`p-1.5 rounded transition-colors ${replayProgress !== -1 ? 'text-primary-500 bg-primary-50 dark:bg-primary-900/30' : 'text-slate-500 hover:text-primary-500 hover:bg-slate-100 dark:hover:bg-slate-700'}`} onClick={() => {
               if (replayProgress === -1) {
                 setReplayProgress(1);
              ...
              <Timer className={`h-4 w-4 ${replayProgress !== -1 ? 'animate-spin-slow' : ''}`} />
            </button>
          </div>

          <div className="flex flex-col rounded-lg border border-white/60 dark:border-slate-700/50 bg-white/70 dark:bg-slate-800/70 shadow-sm backdrop-blur p-1">
            <button type="button" title="Phóng to" className="p-1.5 text-slate-500 hover:text-primary-500 hover:bg-slate-100 dark:hover:bg-slate-700 rounded transition-colors" onClick={() => {
               const currentZoom = fgRef.current?.zoom() || 1;
               fgRef.current?.zoom(currentZoom * 1.5, 400);
            }}>
              <ZoomIn className="h-4 w-4" />
            </button>
            <button type="button" title="Thu nhỏ" className="p-1.5 text-slate-500 hover:text-primary-500 hover:bg-slate-100 dark:hover:bg-slate-700 rounded transition-colors" onClick={() => {
               const currentZoom = fgRef.current?.zoom() || 1;
               fgRef.current?.zoom(currentZoom / 1.5, 400);
            }}>
              <ZoomOut className="h-4 w-4" />
            </button>
            <button type="button" title="Vừa vặn màn hình" className="p-1.5 text-slate-500 hover:text-primary-500 hover:bg-slate-100 dark:hover:bg-slate-700 rounded transition-colors" onClick={() => fgRef.current?.zoomToFit(400, 50)}>
              <Target className="h-4 w-4" />
            </button>
          </div>
          
          <div className="flex flex-col rounded-lg border border-white/60 dark:border-slate-700/50 bg-white/70 dark:bg-slate-800/70 shadow-sm backdrop-blur p-1">
            <button type="button" title={isPhysicsActive ? "Dừng mô phỏng" : "Tiếp tục mô phỏng"} className={`p-1.5 rounded transition-colors ${!isPhysicsActive ? 'text-primary-500 bg-primary-50 dark:bg-primary-900/30' : 'text-slate-500 hover:text-primary-500 hover:bg-slate-100 dark:hover:bg-slate-700'}`} onClick={() => {
            ...
            <button type="button" title="Toàn màn hình" className="p-1.5 text-slate-500 hover:text-primary-500 hover:bg-slate-100 dark:hover:bg-slate-700 rounded transition-colors" onClick={toggleFullscreen}>
        ...
        <div className="absolute bottom-4 right-4 z-20 flex items-center gap-3 rounded-lg border border-white/60 dark:border-slate-700/50 bg-white/70 dark:bg-slate-800/70 px-3 py-2 text-[10px] font-medium text-slate-600 dark:text-slate-300 shadow-sm backdrop-blur">
          <span className="flex items-center gap-1.5"><i className="inline-block h-2.5 w-2.5 rounded-full" style={{ backgroundColor: levelColors.TOPIC }}/> Topic</span>
          <span className="flex items-center gap-1.5"><i className="inline-block h-2.5 w-2.5 rounded-full" style={{ backgroundColor: levelColors.CONCEPT }}/> Concept</span>
          <span className="flex items-center gap-1.5"><i className="inline-block h-2.5 w-2.5 rounded-full" style={{ backgroundColor: levelColors.SKILL }}/> Skill</span>
        </div>
      </div>
    </div>
  );
...
export default function ObsidianPage() {
  ...
  const updateParentMutation = useMutation({
    ...
    onError: (error: unknown) => {
      alert(error instanceof Error ? error.message : "Không cập nhật được liên kết.");
    }
  });
  ...
  const createLinkMutation = useMutation({
    ...
    onError: (error: unknown) => {
      setIsLinking(false);
      alert(error instanceof Error ? error.message : "Không tạo được link.");
    }
  });
  ...
  const handleCreateNote = (event: FormEvent<HTMLFormElement>) => {
    ...
    const parent_id = noteParentId === "none" ? undefined : parseInt(noteParentId, 10);
  ...
  return (
    <div className={`space-y-6 transition-colors duration-300 ${isDarkMode ? 'dark' : ''}`}>
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gradient pb-1">Cấu trúc Kiến thức</h1>
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">
            Không gian trực quan để khám phá các chủ đề, liên kết và ngữ cảnh tri thức trong hệ thống.
          </p>
        </div>
        <motion.button
          ...
          onClick={() => {
            if (!showNewNote) {
              setNoteParentId(selectedNode ? String(selectedNode.entity_id) : "none");
            }
        ...
        >
          <Plus className={`h-4 w-4 mr-2 transition-transform duration-300 ${showNewNote ? 'rotate-45' : ''}`} />
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { label: "Tổng số Node", value: allNodes.length, icon: FileText, color: "text-blue-500 dark:text-blue-400", bg: "bg-blue-500/10 dark:bg-blue-400/10" },
          { label: "Tổng liên kết", value: allEdges.length, icon: LinkIcon, color: "text-purple-500 dark:text-purple-400", bg: "bg-purple-500/10 dark:bg-purple-400/10" },
          {
            label: "Node có câu hỏi",
            value: allNodes.filter((node) => node.question_count > 0).length,
            icon: CheckCircle2, color: "text-emerald-500 dark:text-emerald-400", bg: "bg-emerald-500/10 dark:bg-emerald-400/10"
          },
          {
            label: "Đang chọn",
            value: selectedNode ? selectedNode.label : "-",
            icon: Share2, color: "text-amber-500 dark:text-amber-400", bg: "bg-amber-500/10 dark:bg-amber-400/10"
          },
        ].map((stat, idx) => (
          <motion.div
          ...
          >
            ...
            <div className="relative z-10 flex items-center gap-3">
              <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${stat.bg} ${stat.color} shadow-sm`}>
                <stat.icon className="h-5 w-5" />
              </div>
              <div className="min-w-0">
                <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{stat.label}</p>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 px-6 py-4">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Tạo Node Mới</h3>
                <button type="button" onClick={() => setShowNewNote(false)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-500 dark:hover:bg-slate-800 transition-colors">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="p-6 space-y-5">
                <div>
                  <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-300">Tên Node <span className="text-red-500">*</span></label>
                  <input
                    value={noteName}
                ...
                <div>
                  <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-300">Loại Node</label>
                  <select
                    value={noteType}
                ...
                <div>
                  <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-300">Liên kết tới Node cha</label>
                  <select
                    value={noteParentId}
                ...
                <div>
                  <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-300">Mô tả (tuỳ chọn)</label>
                  <textarea
                ...
                {noteError && (
                  <p className="text-sm font-medium text-red-500">{noteError}</p>
                )}
              </div>
              <div className="flex items-center justify-end gap-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-6 py-4">
                <button type="button" onClick={() => setShowNewNote(false)} className="rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-200 dark:text-slate-300 dark:hover:bg-slate-800 transition-colors">
                  Hủy bỏ
                </button>
                <button type="submit" disabled={createNoteMutation.isPending} className="rounded-xl bg-primary-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-primary-500 disabled:opacity-50 transition-colors shadow-lg shadow-primary-500/30">
        ...
        <aside className="flex flex-col overflow-hidden h-[600px] glass-card shadow-lg border border-slate-200/60 dark:border-slate-700/60 rounded-2xl">
          <div className="p-4 border-b border-slate-200/50 dark:border-slate-800/50 shrink-0">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-extrabold text-slate-900 dark:text-white uppercase tracking-wider">Danh sách Node</h2>
          ...
          {selectedNode ? (
              ...
              >
                <div>
                  ...
                  {selectedNode.description && (
                    <p className="mt-2 text-xs text-slate-500 dark:text-slate-400 italic">{selectedNode.description}</p>
                  )}
                </div>
                  ...
                  {isEditingNote ? (
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => { setIsEditingNote(false); setEditingNote(null); }} className="flex-1 rounded-lg bg-slate-200 dark:bg-slate-800 px-3 py-1.5 text-xs font-semibold hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">Hủy</button>
                        <button
                          onClick={() => updateNoteMutation.mutate({ id: selectedNode.entity_id, note: editingNote?.trim() || null })}
                        ...
                        >
                          {updateNoteMutation.isPending ? "Lưu..." : "Lưu ghi chú"}
                        </button>
                      </div>
                    </div>
                  ) : selectedNode.note ? (
                    <p className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{selectedNode.note}</p>
                  ) : (
                ...
                <div className="pt-3 border-t border-slate-200/50 dark:border-slate-700/50">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    >
                      {isLinking ? "Đang chọn... (Bấm node khác)" : "+ Tạo link"}
                    </button>
                  </div>
                  {isLinking && (
                    <div className="mb-3 p-2.5 rounded-lg bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 text-xs text-primary-700 dark:text-primary-300 font-medium">
                      👆 Bấm vào một node KHÁC trên đồ thị hoặc chọn từ danh sách bên trái để tạo link thủ công với "{selectedNode.label}".
                      <button onClick={() => setIsLinking(false)} className="ml-2 underline hover:no-underline">Hủy</button>
                    </div>
                  )}
                  {/* Show manual links */}
                  {allEdges.filter(e => e.type === "MANUAL" && (
                    (typeof e.source === 'object' ? (e.source as any).id : e.source) === selectedNode.id ||
                    (typeof e.target === 'object' ? (e.target as any).id : e.target) === selectedNode.id
                  )).map(edge => {
                    const otherId = (typeof edge.source === 'object' ? (edge.source as any).id : edge.source) === selectedNode.id
                    ...
                    return otherNode ? (
                      <div key={edge.id} className="flex items-center justify-between rounded-lg bg-white/50 dark:bg-slate-800/50 px-3 py-2 mb-1.5 border border-slate-200/50 dark:border-slate-700/50">
                        <button onClick={() => setSelectedId(otherNode.id)} className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                          <div className="h-2 w-2 rounded-full" style={{ backgroundColor: levelColors[otherNode.type] ?? DEFAULT_NODE_COLOR }} />
                          {otherNode.label}
                        </button>
                        <button
                          onClick={() => edge.link_id && deleteLinkMutation.mutate(edge.link_id)}
                  ...
                  <div className="space-y-2">
                    {connectedNodes.length === 0 && (
                      <div className="flex flex-col items-center justify-center p-6 bg-slate-50/50 dark:bg-slate-800/30 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">Không có liên kết nào</p>
                      </div>
                    )}
                    {connectedNodes.map((node) => (
                      ...
                      >
                        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg shadow-sm transition-transform group-hover:scale-110"
                             style={{ backgroundColor: `${levelColors[node.type] ?? DEFAULT_NODE_COLOR}15`, color: levelColors[node.type] ?? DEFAULT_NODE_COLOR }}>
                          <div className="h-2 w-2 rounded-full" style={{ backgroundColor: levelColors[node.type] ?? DEFAULT_NODE_COLOR }} />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-semibold text-slate-700 dark:text-slate-200 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{node.label}</p>
                        </div>
                ...
                <div className="pt-4 border-t border-slate-200/50 dark:border-slate-700/50 flex flex-col gap-3">
                  ...
                  {isEditingParent ? (
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => setIsEditingParent(false)} className="flex-1 rounded-lg bg-slate-200 dark:bg-slate-800 px-3 py-1.5 text-xs font-semibold hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">Hủy</button>
                        <button 
                          onClick={() => updateParentMutation.mutate({ id: parseInt(selectedNode.entity_id), parent_id: newParentId === "none" ? null : parseInt(newParentId) })} 
                  ...
                  <div className="pt-2 border-t border-red-200/30 dark:border-red-900/30 mt-2">
                     <button
                       onClick={() => {
                         if (window.confirm(`Bạn có chắc chắn muốn xóa node "${selectedNode.label}" không? Tất cả liên kết với node con sẽ bị ngắt.`)) {
                           setIsDeleting(true);
                           deleteNoteMutation.mutate(parseInt(selectedNode.entity_id));
                         }
                       }}
                       disabled={isDeleting}
            ...
            <div className="flex flex-col items-center justify-center h-full text-center opacity-60">
              ...
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
                Chọn một node trên graph<br />để xem chi tiết và liên kết
              </p>
            </div>
          )}
        </aside>
