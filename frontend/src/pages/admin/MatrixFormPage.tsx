...
import type { KnowledgeNode, MatrixRule } from "../../types";
...
export default function MatrixFormPage() {
  ...
  // Omit id for rule creation/editing state
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <textarea
                rows={2}
                className="w-full px-4 py-2 text-sm bg-slate-50/50 dark:bg-slate-950/50 border border-slate-200/60 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all outline-none"
                value={description}
          ...
          <div className="space-y-4">
            ...
            {rules.length === 0 ? (
              ...
              <div className="space-y-4">
                {rules.map((rule, idx) => (
                  <div key={idx} className="p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-white/5 rounded-xl flex flex-col md:flex-row gap-4 items-end">
                    
                    <div className="w-full md:w-1/3 space-y-1">
                      <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Chủ đề kiến thức</label>
                      <select
                        required
                        className="w-full px-3 py-2 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none"
