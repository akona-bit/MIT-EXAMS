...
export async function getExams(skip: number = 0, limit: number = 50, status?: string): Promise<PaginatedResponse<Exam>> {
  const params = new URLSearchParams();
  params.append('skip', skip.toString());
  params.append('limit', limit.toString());
  if (status) params.append('status', status);

  const response = await client.get<PaginatedResponse<Exam>>(`/api/v1/exams/?${params.toString()}`);
...
export async function generateExamForms(id: number, formCount: number): Promise<ExamForm[]> {
  const response = await client.post<ExamForm[]>(`/api/v1/exams/${id}/generate?form_count=${formCount}`);
  return response.data;
}
...
export async function assignParticipants(id: number, userIds: number[]): Promise<any> {
